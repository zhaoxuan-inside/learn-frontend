...
### 2. 网络请求
    ...
    2.2. 即使请求成功，脚本也无法读取响应内容；
        - 
    2.3. Cookie、LocalStorage、SessionStorage、IndexedDB
        - 读取或设置另一个源的 Cookie、Web Storage 数据会被拒绝。
        - 每个源的存储区域完全隔离，无法跨域访问。
        - ⭐ 注意：Cookie 可以通过 domain 属性设置为父域以在子域间共享，但仍是同域限制的一种放宽。
    2.4. WebSocket
        - WebSocket 协议本身没有同源限制，但浏览器在建立连接时会发送 **Origin** 头，**服务器可以据此拒绝跨域握手**。
    2.5. 脚本加载
        - 允许跨域加载 JavaScript 文件，并且脚本可以执行。这是 JSONP 的原理。
        - 脚本本身无法读取跨域资源的原始内容
    2.6. Canvas 图像数据
        - 如果 Canvas 中绘制了跨域图片(例如 <img crossorigin="anonymous">) 且**服务器支持 CORS**，则可以通过 getImageData/toDataURL 读取像素。
